module.exports = {
    ...
    execute: async ({ api, event, args }) => {
        ...
        // The Local Database of all 48 Laws
        const laws = {
        ...
        // Logic: Check if user provided a number
        if (args[0]) {
            lawNumber = parseInt(args[0]);
            
            // Validation: Ensure it is a number and between 1-48
            if (isNaN(lawNumber) || lawNumber < 1 || lawNumber > 48) {
                return api.sendMessage("⚠️ Invalid input. There are only 48 laws. Please choose a number between 1 and 48.", threadID, messageID);
            }
        } else {
            // No number provided? Pick a random one
            lawNumber = Math.floor(Math.random() * 48) + 1;
        }

        // Retrieve data and send message
